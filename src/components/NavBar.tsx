...
export const NavBar = () => {
  ...
  }, [inputRef, searchText, profileMenuRef]);


  return (
    <>
      <nav className="fixed top-0 left-0 right-0 z-50 p-3 text-white shadow-cyan-800 shadow-lg">
        <div className="mx-auto flex items-center justify-between">
          ...
          <div className="relative flex items-center space-x-2" ref={profileMenuRef}>
            <span className="font-semibold text-lg hidden md:block">Welcome {user?.name}</span>
            <button className="p-0 rounded-full focus:outline-none" onClick={handleProfileClick} aria-label="Open Profile Menu">
              <img
            ...
            {showProfileMenu && (
              <div className="absolute right-0 top-full mt-2 w-48 bg-gray-800 bg-opacity-95 backdrop-blur-md rounded-lg shadow-xl overflow-hidden z-50">
                <Link href="/home/profile" onClick={() => setShowProfileMenu(false)} className="flex items-center p-3 hover:bg-gray-700 transition-colors duration-200">
                  <img
                    src={ defaultDp}
                    alt={user?.name}
      ...
      {showSearchResults && (
        <div className="absolute top-[80px] left-0 right-0 z-40 bg-white p-4 pt-8 shadow-lg max-h-64 overflow-y-auto md:max-w-md mx-auto rounded-b-lg">
          {searchResults.length > 0 ? (
            <div className="max-w-md mx-auto">
              {searchResults.map((result) => (
                <Link href={`/home/profile/${result.uid}`} key={result.uid} onClick={() => {
                  setShowSearchResults(false);
                  setSearchText('');
                  setSearchResults([]);
                  setShowSearchInput(false);
